document.addEventListener('DOMContentLoaded', () => {
    ...
    async function obtenerPreguntas() {
        try {
            const response = await fetch('../json/analitica_de_datos.json');
    ...
    function mostrarResultadoFinal() {
        ...
        let resultadosAnteriores = JSON.parse(localStorage.getItem('analitica_de_datos_resultados')) || [];
        resultadosAnteriores.push({
            puntaje: puntaje.toFixed(2),
            fecha: new Date().toLocaleString()
        });
        localStorage.setItem('analitica_de_datos_resultados', JSON.stringify(resultadosAnteriores));

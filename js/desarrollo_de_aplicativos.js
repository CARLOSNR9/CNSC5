document.addEventListener('DOMContentLoaded', () => {
    ...
    async function obtenerPreguntas() {
        try {
            const response = await fetch('../json/desarrollo_de_aplicativos.json');
    ...
    function mostrarResultadoFinal() {
        ...
        let resultadosAnteriores = JSON.parse(localStorage.getItem('desarrollo_de_aplicativos_resultados')) || [];
        resultadosAnteriores.push({
            puntaje: puntaje.toFixed(2),
            fecha: new Date().toLocaleString()
        });
        localStorage.setItem('desarrollo_de_aplicativos_resultados', JSON.stringify(resultadosAnteriores));

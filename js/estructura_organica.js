document.addEventListener('DOMContentLoaded', () => {
    ...
    async function obtenerPreguntas() {
        try {
            const response = await fetch('../json/estructura_organica.json');
    ...
    function mostrarResultadoFinal() {
        ...
        let resultadosAnteriores = JSON.parse(localStorage.getItem('estructura_organica_resultados')) || [];
        resultadosAnteriores.push({
            puntaje: puntaje.toFixed(2),
            fecha: new Date().toLocaleString()
        });
        localStorage.setItem('estructura_organica_resultados', JSON.stringify(resultadosAnteriores));

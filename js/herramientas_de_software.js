document.addEventListener('DOMContentLoaded', () => {
    ...
    async function obtenerPreguntas() {
        try {
            const response = await fetch('../json/herramientas_de_software.json');
    ...
    function mostrarResultadoFinal() {
        ...
        let resultadosAnteriores = JSON.parse(localStorage.getItem('herramientas_de_software_resultados')) || [];
        resultadosAnteriores.push({
            puntaje: puntaje.toFixed(2),
            fecha: new Date().toLocaleString()
        });
        localStorage.setItem('herramientas_de_software_resultados', JSON.stringify(resultadosAnteriores));

document.addEventListener('DOMContentLoaded', () => {
    ...
    async function obtenerPreguntas() {
        try {
            const response = await fetch('../json/sistemas_de_informacion.json');
    ...
    function mostrarResultadoFinal() {
        ...
        let resultadosAnteriores = JSON.parse(localStorage.getItem('sistemas_de_informacion_resultados')) || [];
        resultadosAnteriores.push({
            puntaje: puntaje.toFixed(2),
            fecha: new Date().toLocaleString()
        });
        localStorage.setItem('sistemas_de_informacion_resultados', JSON.stringify(resultadosAnteriores));
